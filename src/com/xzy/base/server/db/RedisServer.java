...
public class RedisServer extends BasicServer {
    ...
    public boolean startServer() {
        ...
        if (obj instanceof String) {
            ...
            this.singlePool = new JedisPool(config, arr[0],
                    Integer.parseInt(arr[1]), timeout, passwd);
        } else {
        ...
        this.redisDataListenerPoolName = this.getServerName()
                + "-redisListener";
        ThreadPoolInfo poolInfo = new ThreadPoolInfo(1);
        this.isRun = ThreadPoolServer.getSingleInstance()
            .createThreadPool(this.redisDataListenerPoolName, poolInfo);
        ThreadPoolServer.getSingleInstance().addTask2ThreadPool(
            this.redisDataListenerPoolName,
    ...
    public void stopServer() {
        ...
        if (this.jedisObserver != null) {
            synchronized (this.jedisObserver) {
                this.jedisObserver.notifyAll();
            }
        }

        ThreadPoolServer.getSingleInstance()
            .removeThreadPool(this.redisDataListenerPoolName);
    ...
    public String setValue(String key, String value, Date expiredTime,
        ...
        long duration = 0;
        if (expiredTime != null) {
            duration = (expiredTime.getTime() - System.currentTimeMillis())
                    / 1000;
        }
    ...
    public String setValue(String key, String value,
            int existDurationWithSeconds) {
        return this.setValue(key, value, existDurationWithSeconds,
            this.defaultDbIndex);
    }
     ...
     */
    public String setValue(String key, String value,
            int existDurationWithSeconds, int dbIndex) {
        if (!this.isRunning()) {
            return null;
        }
        if (this.singlePool != null) {
     ...
     * @return
     */
    public synchronized boolean requestLock(String guid, String lockName,
        ...
        LockInfo lock = new LockInfo(guid, lockName,
                expireDurationSeconds * 1000);
    ...
    public synchronized boolean releaseLock(String guid, String lockName) {
        LockInfo lock = (LockInfo) this.lockMapping
            .remove(guid + "\n" + lockName);
        if (lock != null) {
            return lock.release();
        }
    ...
    private class JedisObserver extends BasicTask {
        ...
        public JedisObserver() {
            this.observer = new JedisPubSub() {
                ...
                @Override
                public void onPMessage(String pattern, String msg,
                        String content) {
        ...
        public synchronized void rebuild(List _patternList) {
            ...
            if (this.observer.isSubscribed()) {
                if (oldPatterList != null) {
                    ...
                    for (Iterator itr = oldPatterList.iterator(); itr
                        .hasNext();) {
                        p = (String) itr.next();
                    ...
                    opList.clear();
                    for (Iterator itr = this.patternList.iterator(); itr
                        .hasNext();) {
                        p = (String) itr.next();
        ...
        public void run() {
            while (isRunning()) {
                ...
                if (RedisServer.this.singlePool != null) {
                    ...
                    Iterator<Map.Entry<String, JedisPool>> itr = RedisServer.this.clusterPool
                        .getClusterNodes().entrySet().iterator();
                    while (itr.hasNext()) {
                        entry = itr.next();
                        try {
    ...
    private class LockInfo {
        ...
        public boolean request() {
            long expireAtTime = System.currentTimeMillis()
                    + this.expireDuration;
            ...
            try {
                ...
                if (infoArr.length > 1 && Long.parseLong(infoArr[1]) < System
                    .currentTimeMillis()) {
                    // 判断是否为空，不为空的情况下，如果被其他线程设置了值，则第二个条件判断是过不去的
                    // lock is expired
                    String oldValueStr = RedisServer.this.getSet(lockName,
                        infoStr);
